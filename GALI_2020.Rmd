...
#Panel data structure. Revenues in year t-1 as revenue 
```{r}
revenue_data <- select(gali_joined, starts_with('country') | starts_with("attention") | starts_with("inst") | ends_with("fins_revenues_m1") | ends_with("External_ID"))

revenue_data <- select(revenue_data, -fu4fins_revenues_m1)

revenue_data <- revenue_data %>% 
  gather(year, revenue, ends_with("fins_revenues_m1"))

revenue_data$year[revenue_data$year == "fins_revenues_m1"] <- -1
revenue_data$year[revenue_data$year == "fu1fins_revenues_m1"] <- 0
revenue_data$year[revenue_data$year == "fu2fins_revenues_m1"] <- 1
revenue_data$year[revenue_data$year == "fu3fins_revenues_m1"] <- 2
...
#Initial FE regression without controls 
```{r}
revenue_data <- transform(revenue_data, id=match(paste(revenue_data$country,revenue_data$New_External_ID,sep="_"), unique(paste(revenue_data$country,revenue_data$New_External_ID,sep="_")))) #create a column called id which assigns a unique id to the unique combinations of origin and destination countries


gali_fe_mod <- plm(revenue ~ (attention_fc*inst_fc_gci) , 
                   data = revenue_data, 
                   index = c("id", "year"),
                   model = "within")
