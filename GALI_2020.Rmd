...
gali$attention_hc <- 1- round(gali$accel_ben_rank_business_skills,2)
...
gali$attention_fc <- 1- round(gali$accel_ben_rank_direct_funding,2)
...
#Join all 3 datasets 
```{r}
gali_joined <- left_join(gali, gem, by = c("country", "year"))
...
```{r}
fit0 <- lmer(capital_raised_tot ~  attention_fc + (attention_fc|country), data = gali_joined, REML = F)

fit1 <- lmer(capital_raised_tot ~ attention_fc + attention_fc*EOSQ425 + (1 |country), data = gali_joined, REML = F)

fit2 <- lmer(capital_raised_tot ~ attention_fc + attention_fc*EOSQ089 + (1 |country), data = gali_joined, REML = F)

fit3 <- lmer(capital_raised_tot ~ attention_fc + attention_fc*inst_fc_gem + (1 |country), data = gali_joined, REML = F)
